...
class AudioRecorder extends React.PureComponent {
    ...
    const drawFrame = (_) => {
      ...
      if (duration > MAX_DURATION) {
        ...
        this.setState({
          enabled: false,
          recording: false,
          duration: secondsToTime(this.durationMillis / 1000),
        });
      }
      ...
      const dx =
        viewDuration > duration
          ? 0
          : ((duration - MILLIS_PER_BAR * barCount) / MILLIS_PER_BAR) *
            (LINE_WIDTH + SPACING);
      ...
      for (let i = 0; i < this.viewBuffer.length; i++) {
        ...
        let y = Math.max(
          Math.min(this.viewBuffer[i] / BAR_SCALE, 0.9) * height,
          1
        );
  ...
  initMediaRecording(stream) {
    ...
    if (!this.mediaRecorder) {
      console.warn(
        "MediaRecorder failed to initialize: no supported audio formats"
      );
      this.props.onError(
        this.props.intl.formatMessage(messages.failed_to_init_audio)
      );
      return;
    }
    ...
    if (!this.audioInput) {
      console.warn(
        "createMediaStreamSource returned null: audio input unavailable"
      );
      this.props.onError(
        this.props.intl.formatMessage(messages.failed_to_init_audio)
      );
    ...
    this.mediaRecorder.onstop = (_) => {
      if (this.durationMillis > MIN_DURATION) {
        this.getRecording(this.mediaRecorder.mimeType).then((result) =>
          this.props.onFinished(result.url, result.preview, this.durationMillis)
        );
      } else {
  ...
  render() {
    ...
    const resumeClass =
      "material-icons " + (this.state.enabled ? "red" : "gray");
    return (
      <div className="audio">
        <a
          href="#"
