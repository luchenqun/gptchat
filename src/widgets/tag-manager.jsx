...
export default class TagManager extends React.Component {
  ...
  handleAddTag(tag) {
    const maxTagCount = this.props.tinode.getServerParam(
      "maxTagCount",
      MAX_TAG_COUNT
    );
  ...
  render() {
    const minTagLength = this.props.tinode.getServerParam(
      "minTagLength",
      MIN_TAG_LENGTH
    );
    const maxTagLength = this.props.tinode.getServerParam(
      "maxTagLength",
      MAX_TAG_LENGTH
    );
    ...
    if (this.state.activated) {
      ...
      this.state.tags.map((tag) => {
        tags.push(
          <span className="badge" key={tags.length}>
            {tag}
          </span>
        );
      });
