...
const messages = defineMessages({
  already_in_call: {
    ...
    description:
      "Error message when the user tried to accept a new call without finishing pervious one",
  },
});
...
class CallPanel extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    if (
      (this.props.callState == CALL_STATE_OUTGOING_INITATED ||
        this.props.callState == CALL_STATE_IN_PROGRESS) &&
  ...
  start() {
    if (this.state.localStream) {
      this.props.onError(
        this.props.intl.formatMessage(messages.already_in_call),
        "info"
      );
      return;
    }
    ...
    if (this.state.pc) {
      ...
      if (
        ...
        (this.state.dataChannel.readyState == "open" ||
          this.state.dataChannel.readyState == "connecting")
      ) {
  ...
  handleVideoAnswerMsg(info) {
    ...
    this.state.pc
      ...
      .then((_) => {
        this.setState({ callInitialSetupComplete: true }, (_) =>
          this.drainRemoteIceCandidatesCache()
        );
      })
  ...
  render() {
    const audioTracks =
      this.state.localStream && this.state.localStream.getAudioTracks();
    const videoTracks =
      !this.state.audioOnly &&
      this.state.localStream &&
      this.state.localStream.getVideoTracks();
    const disabled =
      !this.state.pc ||
      !this.state.dataChannel ||
      !(audioTracks && audioTracks[0]);
    const audioIcon =
      audioTracks && audioTracks[0] && audioTracks[0].enabled
        ? "mic"
        : "mic_off";
    const videoIcon =
      videoTracks &&
    ...
    if (
      this.remoteRef.current &&
      this.remoteRef.current.srcObject &&
      this.state.remoteVideoLive
    ) {
