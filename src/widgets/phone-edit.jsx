...
class PhoneEdit extends React.PureComponent {
  ...
  handleChange(e) {
    ...
    let formatted = new AsYouType().input(
      `${prefix}${this.filterNumber(e.target.value)}`
    );
  ...
  handleFinished(e) {
    ...
    const raw = `${
      this.state.dialCode
    }${this.state.localNumber.trim()}`.replace(/[^\d]/g, "");
    let number = null;
    try {
    ...
    if (!number || !number.isValid()) {
      this.inputField.setCustomValidity(
        this.props.intl.formatMessage(messages.mobile_number_required)
      );
      return;
    }
  ...
  showCountrySelector() {
    this.props.onShowCountrySelector(
