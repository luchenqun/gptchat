...
class BaseChatMessage extends React.PureComponent {
  ...
  handleContextClick(e) {
    ...
    this.props.showContextMenu(
      {
        seq: this.props.seq,
        replace: this.props.edited
          ? parseInt(this.props.edited.split(":")[1])
          : 0,
  ...
  render() {
    ...
    const attachments = [];
    if (
      this.props.mimeType == Drafty.getContentType() &&
      Drafty.isValid(content)
    ) {
      Drafty.attachments(
        content,
        (att, i) => {
...
const ChatMessage = React.forwardRef((props, ref) => (
  <IntlChatMessage innerRef={ref} {...props} />
));
