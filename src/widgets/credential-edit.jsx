...
class CredentialEdit extends React.PureComponent {
  ...
  handleSubmit(e) {
    e.preventDefault();
    const value =
      this.props.method == "email" ? this.state.email : this.state.tel;
    if (value) {
  ...
  render() {
    ...
    const currentValue = CredentialEdit.formatPhoneNumber(
      readyForCode ? this.state.tel || this.state.email : this.props.val
    );
