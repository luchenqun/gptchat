...
const messages = defineMessages({
  search_for_contacts: {
    ...
    description:
      "Text shown in contacts view when user entered no search query.",
  },
  search_no_results: {
...
class NewTopicView extends React.Component {
  ...
  handleSearchResultSelected(topicName) {
    if (this.state.tabSelected == "find") {
      HashNavigation.navigateTo(
        HashNavigation.removeUrlParam(window.location.hash, "tab")
      );
  ...
  handleNewGroupSubmit(name, description, dataUrl, priv, tags, isChannel) {
    ...
    this.props.onCreateTopic(
      undefined,
      {
        public: theCard(name, dataUrl, null, description),
        private: priv,
        tags: tags,
      },
      isChannel
